...
export const useMusicStore = defineStore("music", {
  ...
  getters: {
    ...
    // 私人 FM 数据（从 data store 获取）
    personalFM(): { playIndex: number; list: SongType[] } {
      const dataStore = useDataStore();
      return dataStore.currentPersonalFM;
    },
    // 每日推荐数据（从 data store 获取）
    dailySongsData(): { timestamp: number | null; list: SongType[] } {
      const dataStore = useDataStore();
      return dataStore.currentDailySongsData;
    },
    // 私人FM播放歌曲
    personalFMSong(): SongType {
      return this.personalFM.list?.[this.personalFM.playIndex] || defaultMusicData;
    },
  },
  actions: {
    // 恢复默认音乐数据

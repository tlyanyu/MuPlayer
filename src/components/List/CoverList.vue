<template>
  <Transition name="fade" mode="out-in">
    <div v-if="data.length > 0" :class="['cover-list', type]">
      <n-grid :cols="cols" x-gap="20" y-gap="20">
        <n-gi v-for="(item, index) in data" :key="index">
          ...
          >
            ...
            <div class="cover">
              <!-- 平台图标 - 左上角 -->
              <div v-if="item.platform" class="platform-badge">
      ...
      <!-- 加载更多 -->
      <div v-if="loadMore" ref="loadMoreRef" class="load-more">
        <n-flex justify="center">
          <n-spin v-if="loading" size="medium" />
          <n-text v-else depth="3">滚动加载更多...</n-text>
        </n-flex>
      </div>
      <!-- 右键菜单 -->
...
const playList = debounce(
  async (item: CoverType) => {
    try {
      ...
      item.loading = true;
      // 获取播放列表
      const list = await getListData(item.id, item.platform);
...
// 获取列表数据
const getListData = async (id: number, platform?: string): Promise<SongType[]> => {
  const options = platform ? { platform: platform as Platform } : undefined;
  switch (props.type) {
    case "album": {
      const result = await albumDetail(id, options);
      const ids: number[] = result.songs.map((song: any) => song.id as number);
      const songRes = await songDetail(ids, options);
      return formatSongsList(songRes.songs, platform as Platform);
    }
    case "playlist": {
      // 仅请求 100 首
      const result = await playlistAllSongs(id, 100, 0, options);
      return formatSongsList(result.songs, platform as Platform);
    }
    case "radio": {

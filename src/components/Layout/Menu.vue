...
import { useStatusStore, useSettingStore, useDataStore, useMusicStore, usePlatformStore } from "@/stores";
...
const menuOptions = computed<MenuOption[] | MenuGroupOption[]>(() => {
  return settingStore.useOnlineService
    ? [
        ...
        // 根据平台显示不同模块：网易云显示播客电台，QQ音乐显示视频
        platformStore.currentPlatform === Platform.NETEASE
          ? {
              key: "radio-hot",
              link: "radio-hot",
              label: "播客电台",
        ...
        },
        {
          ...
          link: "cloud",
          label: "我的云盘",
          show: isLogin() === 1 && platformStore.currentPlatform !== Platform.QQMUSIC,
...
const createPlaylist = computed<MenuOption[]>(() => {
  ...
  const list = dataStore.currentUserLikeData.playlists
...
const likedPlaylist = computed<MenuOption[]>(() => {
  const userId = dataStore.currentUserData.userId;
  const list = dataStore.currentUserLikeData.playlists.filter((playlist) => playlist?.userId !== userId);
  return renderPlaylist(list, settingStore.menuShowCover);
});
...
const checkMenuItem = () => {
  ...
  switch (routerName) {
    case "playlist": {
      ...
      // 是否处于用户歌单
      const isUserPlaylist = dataStore.currentUserLikeData.playlists.some(
...
// 监听路由
watch(
  () => [router.currentRoute.value, dataStore.currentUserLikeData.playlists],
  () => checkMenuItem(),
);
</script>

...
const getHotCommentData = async () => {
  ...
  // 获取评论
  const result = await getHotComment(
    songId.value,
    songType.value,
    20,
    0,
    undefined,
    { resource: musicStore.playSong },
  );
  // 处理数据（传递平台信息）
  const formatData = formatCommentList(result.hotComments || result.data?.comments, musicStore.playSong.platform);
...
const getAllComment = async () => {
  ...
  // 分页参数：第一页传空字符串，后续页传 cursor
  const cursor = commentPage.value > 1 && commentCursor.value ? commentCursor.value : "";
  // 获取评论（传递歌曲对象以支持智能平台路由）
  ...
  // 处理数据（传递平台信息）
  const formatData = formatCommentList(result.data.comments, musicStore.playSong.platform);

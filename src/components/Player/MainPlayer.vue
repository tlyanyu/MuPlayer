        ...
        <div :key="musicStore.playSong.id" class="info">
          <div class="data">
            ...
            <SvgIcon
              ...
              :name="dataStore.isLikeSong(musicStore.playSong.id, musicStore.playSong.platform) ? 'Favorite' : 'FavoriteBorder'"
              :size="20"
              class="like"
              @click="
                toLikeSong(musicStore.playSong, !dataStore.isLikeSong(musicStore.playSong.id, musicStore.playSong.platform))
              "
            />
            <!-- 更多操作 -->
            ...
            <div v-else class="artists">
              ...
              <template v-else-if="Array.isArray(musicStore.playSong.artists)">
                <n-text
                  ...
                  :key="index"
                  class="ar-item"
                  @click="openJumpArtist(musicStore.playSong.artists, musicStore.playSong.platform)"
                >
                  {{ item.name }}
                </n-text>
              </template>
              <n-text v-else class="ar-item" @click="openJumpArtist(musicStore.playSong.artists, musicStore.playSong.platform)">
    ...
    <Transition name="fade" mode="out-in">
      ...
      >
        ...
        <div class="time">
          <!-- 平台图标 -->
          <PlatformIcon
            v-if="musicStore.playSong.platform"
            :platform="musicStore.playSong.platform"
            :size="24"
            :class="['platform-icon', { spinning: statusStore.playStatus }]"
            clickable
          />
      ...
      props: {
        onClick: () =>
          router.push({ 
            name: "video", 
            query: { 
              id: musicStore.playSong.mv, 
              type: "mv",
              platform: musicStore.playSong.platform // 传递平台信息以支持跨平台 MV 播放
            } 
          }),
      },

  ...
  <div class="song-card">
    <div :class="['song-content', { play: musicStore.playSong.id === song.id }]">
        ...
        <div class="info">
          ...
          <div v-if="Array.isArray(song.artists)" class="artists text-hidden">
            <n-text
              ...
              :key="ar.id"
              class="ar"
              @click="openJumpArtist(song.artists, song.platform)"
            >
              {{ ar.name }}
            </n-text>
          </div>
          ...
          <div v-else class="artists text-hidden" @click="openJumpArtist(song.artists, song.platform)">
        ...
        <n-text
          ...
          @click="
            router.push({
              ...
              query: {
                id: song.album?.id,
                platform: song.platform, // 传递平台信息以支持跨平台专辑访问
              },
            })
          "
        >
      ...
      <div v-if="song.type !== 'radio'" class="actions" @click.stop @dblclick.stop>
        <!-- 喜欢歌曲 -->
        <SvgIcon
          :name="dataStore.isLikeSong(song.id, song.platform) ? 'Favorite' : 'FavoriteBorder'"
          :size="20"
          @click.stop="toLikeSong(song, !dataStore.isLikeSong(song.id, song.platform))"
          @delclick.stop
        />
      </div>
      <!-- 更新日期 -->

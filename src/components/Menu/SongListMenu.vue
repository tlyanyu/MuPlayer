...
import { useStatusStore, useLocalStore, useDataStore, usePlatformStore } from "@/stores";
...
const openDropdown = (
  ...
  try {
    ...
    // 用户歌单
    const userPlaylistsData = dataStore.currentUserLikeData.playlists?.filter(
      (pl) => pl.userId === dataStore.currentUserData.userId,
    );
    // 当前状态
            ...
            onClick: () =>
              router.push({
                ...
                query: {
                  id: song.mv,
                  type: "mv",
                  platform: song.platform, // 传递平台信息以支持跨平台 MV 播放
                },
              }),
          },
...
const deleteCloudSongData = (song: SongType, index: number) => {
  window.$dialog.warning({
    ...
    onPositiveClick: async () => {
      ...
      if (result.code == 200) {
        const cloudList = [...dataStore.currentCloudPlayList];
        cloudList.splice(index, 1);
        await dataStore.setPlatformCloudPlayList(platformStore.currentPlatform, cloudList);
...
const importSongToCloud = async (song: SongType) => {
  ...
  if (!songDetail) {
    window.$message.error("获取歌曲信息失败，请重试");
    return;
  }
  // 开始尝试导入
  ...
  const result = await importCloudSong(song?.name, type, size, Math.floor(br / 1000), md5, id, artistName, albumName);
  if (result.code === 200) {

...
const login = debounce(async (e: MouseEvent) => {
  ...
  if (loginResult.cookie) {
    try {
      // Axios已自动解析JSON,cookie字段已是对象
      const cookiesObj = loginResult.cookie;

      // 检查是否含有 MUSIC_U (网易云登录标识)
      if (cookiesObj && typeof cookiesObj === 'object' && cookiesObj.MUSIC_U) {
        // 构建新的登录数据对象
        const loginData = {
        ...
        // 储存登录信息
        emit("saveLogin", loginData, "phone");
      } else {

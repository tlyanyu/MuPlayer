...
const props = defineProps<{ id: number; platform?: string }>();
...
const getSongDetail = async (): Promise<any> => {
  if (!props.id) return;
  const options = props.platform ? { platform: props.platform as Platform } : undefined;
  const result = await songDetail(props.id, options);
  songData.value = formatSongsList(result.songs, props.platform as Platform)[0];
  // 获取音质信息
  const quality = await songQuality(props.id, options);
  console.log(quality);
  // 获取下载信息
...
const download = async () => {
  ...
  try {
    // 获取下载链接
    const options = props.platform ? { platform: props.platform as Platform } : undefined;
    const result = await songDownloadUrl(props.id, songLevelChoosed.value, options);

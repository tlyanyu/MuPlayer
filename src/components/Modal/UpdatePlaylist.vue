...
// 是否为我喜欢
const isLiked = computed(() => dataStore.currentUserLikeData.playlists?.[0]?.id === props.id);
...
const tagList = computed<SelectOption[]>(() => {
  if (isEmpty(dataStore.currentCatData?.cats)) return [];
  return Object.keys(dataStore.currentCatData?.type).map((key) => ({
    type: "group",
    key,
    label: dataStore.currentCatData?.type[key],
    children: dataStore.currentCatData?.cats

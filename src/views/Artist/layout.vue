        ...
        <div class="data">
          ...
          <n-collapse-transition :show="!listScrolling" class="collapse">
            ...
            <n-flex class="meta">
              <div
                class="item"
                @click="router.push({ name: 'artist-songs', query: { id: artistId, platform: artistPlatform } })"
              >
              ...
              <div
                class="item"
                @click="router.push({ name: 'artist-albums', query: { id: artistId, platform: artistPlatform } })"
              >
              ...
              <div
                class="item"
                @click="router.push({ name: 'artist-videos', query: { id: artistId, platform: artistPlatform } })"
              >
...
// 歌手 ID（支持网易云的 number 和 QQ音乐的 string）
const artistId = computed<number | string>(() => {
  const id = router.currentRoute.value.query.id as string;
  // 尝试转换为数字，如果失败则保持字符串（用于QQ音乐等平台）
  const numId = Number(id);
  return isNaN(numId) ? id : numId;
});

// 获取 URL 中的平台信息（支持跨平台艺术家访问）
const artistPlatform = computed(() => {
...
const isLikeArtist = computed(() => {
  // 使用宽松相等来比较 number 和 string 类型的ID
  return dataStore.currentUserLikeData.artists.some((ar) => ar.id == artistId.value);
});
...
const getArtistDetail = async (id: number | string) => {
  try {
    ...
    // 构建平台路由选项（支持跨平台艺术家访问）
    const options = artistPlatform.value ? { platform: artistPlatform.value } : undefined;
    const result = await artistDetail(id, options);
    artistDetailData.value = formatArtistsList(result.data.artist, artistPlatform.value)[0];
    // 附加身份
...
const tabChange = (value: string) => {
  router.push({
    name: value,
    query: { id: artistId.value, platform: artistPlatform.value },
  });
};

// 播放全部歌曲
const playAllSongs = async () => {
  await router.push({ name: "artist-songs", query: { id: artistId.value, platform: artistPlatform.value } });
...
onBeforeRouteUpdate((to) => {
  ...
  // 尝试转换为数字，如果失败则保持字符串
  const numId = Number(id);
  const parsedId = isNaN(numId) ? id : numId;
  if (parsedId && parsedId !== artistId.value) getArtistDetail(parsedId);
});

  ...
  <div class="discover-playlists">
    ...
    >
      ...
      <n-tabs type="segment" animated>
        <n-tab-pane
          v-for="(item, key, index) in dataStore.currentCatData.type"
          :key="index"
          :name="key"
          :tab="item"
        >
          <n-flex class="cat-list">
            <n-tag
              v-for="(cat, catIndex) in dataStore.currentCatData.cats.filter(
...
const hasHqPlaylist = computed<boolean>(() => {
  if (dataStore.currentCatData.hqCats?.length === 0) return false;
  if (catName.value === "全部歌单") return true;
  return dataStore.currentCatData.hqCats.some((item) => item.name === catName.value);
});

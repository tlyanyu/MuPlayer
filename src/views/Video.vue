        ...
        <div
          ...
          @click="
            router.push({
              ...
              query: {
                id: artistData?.id,
                platform: videoPlatform, // 传递平台信息以支持跨平台艺术家访问
              },
            })
          "
        >
      ...
      <n-flex class="title" justify="space-between">
        ...
        <n-flex class="tag">
          <n-tag
            ...
            round
            @click="changeCommentType(key as 'hot' | 'new')"
          >
            {{ item }}
          </n-tag>
        </n-flex>
...
// 视频参数
// 智能类型转换：如果包含非数字字符，保持字符串；否则转为数字
const videoId = computed<number | string>(() => {
  const id = router.currentRoute.value.query.id as string;
  // 如果包含非数字字符（QQ音乐的 mv_vid），保持字符串
  return /[^\d]/.test(id) ? id : Number(id);
});
const videoType = computed<"video" | "mv">(
  () => (router.currentRoute.value.query.type as "video" | "mv") || "mv",
);
// 获取 URL 中的平台信息（支持跨平台 MV 播放）
const videoPlatform = computed(() => {
  return router.currentRoute.value.query.platform as Platform | undefined;
});
...
// 获取视频数据
const getVideoData = async (id: number | string, type: "mv" | "video") => {
  try {
    if (!id || !videoPlayer.value) return;
    
    // 构建平台路由选项（支持跨平台 MV 播放）
    const options = videoPlatform.value ? { platform: videoPlatform.value } : undefined;
    
    // 获取视频详情
    const detail = await videoDetail(id, type, options);
    const detailInfo = await videoDetailInfo(id, type, options);
    // 处理数据
    ...
    const requests = brs.map(async (v: any) => {
      try {
        const result = await videoUrl(id, type, v.br, options);
        return {
...
// 获取评论数据
const getCommentData = async (id: number | string, clean: boolean = true) => {
  try {
    if (!id) return;
    ...
    // 分页参数：使用 API 返回的 cursor
    const cursor = commentPage.value > 1 && commentCursor.value ? commentCursor.value : undefined;
    ...
    // 处理数据（传递平台信息）
    const formatData = formatCommentList(result.data.comments, videoData.value?.platform);
    commentData.value = commentData.value.concat(formatData);
    // 更新 cursor
    if (result.data.cursor) {
      commentCursor.value = result.data.cursor;
    }
    // 是否还有
...
const changeCommentType = (type: "hot" | "new") => {
  if (commentType.value === type) return;
...
// 监听路由更新，支持在同一路由内切换不同视频
onBeforeRouteUpdate((to, from) => {
  // 只在 video 路由内切换时才重新加载
  if (to.name === "video" && to.query.id && to.query.id !== from.query.id) {
    const newId = /[^\d]/.test(to.query.id as string)
      ? (to.query.id as string)
      : Number(to.query.id);
    const newType = (to.query.type as "video" | "mv") || "mv";
    console.log("Video route updated:", from.query.id, "->", newId);
    getVideoData(newId, newType);
  }
});

...
const getSearchResult = async () => {
  // 获取数据
  loading.value = true;
  const result = await searchResult(props.keyword, 30, searchOffset.value, 1);
  // 是否还有
  hasMore.value = result.result?.hasMore || result.result?.songCount > searchOffset.value + 30;
  // 搜索总数

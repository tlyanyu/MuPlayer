...
const getSearchResult = async () => {
  // 获取数据
  loading.value = true;
  const result = await searchResult(props.keyword, 30, searchOffset.value, 100);
  // 是否还有
  hasMore.value = result.result?.hasMore || result.result?.artistCount > searchOffset.value + 30;
  // 搜索总数

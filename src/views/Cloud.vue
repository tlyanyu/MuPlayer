<template>
  <div v-if="!isQQMusicPlatform" class="cloud">
    <div class="title">
...
const isQQMusicPlatform = computed<boolean>(
  () => platformStore.currentPlatform === Platform.QQMUSIC,
);
...
const cloudData = shallowRef<SongType[]>(dataStore.currentCloudPlayList);
...
const getAllCloudMusic = async () => {
  if (isQQMusicPlatform.value || !isCloudPage.value) return;
  const targetPlatform = platformStore.currentPlatform;
  loading.value = true;
  let offset: number = 0;
  ...
  try {
    do {
      if (platformStore.currentPlatform !== targetPlatform || !isCloudPage.value) break;
      ...
      offset += limit;
    } while (offset < cloudCount.value);
    if (platformStore.currentPlatform !== targetPlatform) return;
    await dataStore.setPlatformCloudPlayList(targetPlatform, cloudData.value);
  } finally {
    if (platformStore.currentPlatform === targetPlatform) {
      loading.value = false;
    }
  }
};
...
onActivated(() => {
  ...
  } else if (!isQQMusicPlatform.value) {
    getAllCloudMusic();
  }
});

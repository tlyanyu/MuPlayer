            ...
            <n-flex class="meta">
              <div class="item">
                ...
                <div
                  ...
                  @click="openJumpArtist(albumDetailData.artists, albumDetailData.platform)"
                >
                  <n-text
...
// 获取 URL 中的平台信息（支持跨平台专辑访问）
const albumPlatform = computed(() => {
...
const isLikeAlbum = computed(() =>
  dataStore.currentUserLikeData.albums.some((album) => album.id === albumDetailData.value?.id),
);
...
const getAlbumDetail = async (id: number, refresh: boolean = false) => {
  ...
  // 构建平台路由选项（支持跨平台专辑访问）
  const options = albumPlatform.value ? { platform: albumPlatform.value } : undefined;
  
  // 获取专辑详情
  const detail = await albumDetail(id, options);
  albumDetailData.value = formatCoverList(detail.album, albumPlatform.value)[0];
  // 获取专辑歌曲
  const ids: number[] = detail.songs.map((song: any) => song.id as number);
  const result = await songDetail(ids, options);
  albumData.value = formatSongsList(result.songs, albumPlatform.value);
  loading.value = false;
};

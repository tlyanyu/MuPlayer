...
import { useDataStore, useStatusStore, usePlatformStore } from "@/stores";
...
// 歌单 ID
const playlistId = computed<number>(() => dataStore.currentUserLikeData.playlists?.[0]?.id);
...
const playlistDataShow = computed(() => {
  if (searchValue.value) return searchData.value;
  // 优先使用 store 中的实时数据，保持响应式连接
  // 如果 store 中有数据，使用 store 的数据（会实时响应 toLikeSong 的更新）
  // 否则使用本地加载的数据（兼容初次加载场景）
  return dataStore.currentLikeSongsList.data.length > 0
    ? dataStore.currentLikeSongsList.data
    : playlistData.value;
});
...
const getPlaylistData = async (id: number, getList: boolean, refresh: boolean) => {
  ...
  playlistDetailData.value = formatCoverList(detail.playlist, platformStore.currentPlatform)[0];
  // 不需要获取列表或无歌曲
  ...
  const count = playlistDetailData.value?.count as number;
  if (isLogin() === 1 && detail.playlist?.tracks?.length && count < 800) {
    const ids: number[] = detail.playlist.tracks.map((song: any) => song.id as number);
  ...
  await dataStore.setPlatformLikeSongsList(
    ...
    playlistDetailData.value,
    playlistData.value,
  );
  // 同步更新喜欢的歌曲 ID 列表，确保红心状态正确显示
  const likedSongIds = playlistData.value.map((song) => song.id);
  dataStore.setPlatformUserLikeData(platformStore.currentPlatform, "songs", likedSongIds);
  loading.value = false;
};

// 加载缓存
const loadLikedCache = () => {
  if (isObject(dataStore.currentLikeSongsList.detail)) {
    playlistDetailData.value = dataStore.currentLikeSongsList.detail;
  }
  if (dataStore.currentLikeSongsList.data.length) {
    playlistData.value = dataStore.currentLikeSongsList.data;
  }
};
...
const getPlaylistAllSongs = async (
  ...
  do {
    ...
    const songData = formatSongsList(result.songs, platformStore.currentPlatform);
...
onMounted(async () => {
  // 使用 playlistId 获取正确的歌单 ID（从 userLikeData.playlists[0].id）
  if (playlistId.value) {

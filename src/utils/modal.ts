...
export const openUserLogin = (showTip: boolean = false) => {
  ...
  const platformName = platformStore.currentPlatformName;
  
  const modal = window.$modal.create({
    preset: "card",
    title: `登录 ${platformName}`,
    transformOrigin: "center",
    autoFocus: false,
    maskClosable: false,
    closeOnEsc: false,
...
// 跳转到歌手
export const openJumpArtist = (data: SongType["artists"], platform?: string) => {
...
export const openDownloadSong = (song: SongType) => {
  ...
  if (song.free !== 0 && dataStore.currentUserData.vipType === 0 && !song?.pc) {
  ...
  const modal = window.$modal.create({
    ...
    title: "下载歌曲",
    content: () => {
      return h(DownloadSong, { id: song.id, platform: song.platform, onClose: () => modal.destroy() });
    },
  });
};

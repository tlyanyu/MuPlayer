...
/**
 * 智能确定资源的平台来源
 * 优先级：后端返回 > 手动传入 > 本地判断 > API模式默认 > 当前平台fallback
 */
const determinePlatform = (item: any, manualPlatform?: Platform): Platform | undefined => {
  // 后端返回的platform字段
  if (item.platform) {
    return item.platform as Platform;
  }
  ...
  return platformStore.currentPlatform;
};

// 格式化歌曲列表
export const formatSongsList = (data: any[], platform?: Platform): SongType[] => {
  if (!data) return [];
  ...
  return data.map((item) => {
    ...
    return {
      ...
      platform: determinePlatform(item, platform),
    };
  });
};

// 格式化封面列表
export const formatCoverList = (data: any[], platform?: Platform): CoverType[] => {
  if (!data) return [];
  data = isArray(data) ? data : [data];
  // 过滤掉 null/undefined 元素,防止创建无效对象
  return data
    .filter(Boolean)
    .map((item) => {
    // 处理数据
...
// 格式化歌手列表
export const formatArtistsList = (data: any[], platform?: Platform): ArtistType[] => {
  if (!data) return [];
  data = isArray(data) ? data : [data];
  // 过滤掉 null/undefined 元素,防止创建无效对象
  return data
...
// 格式化评论列表
export const formatCommentList = (data: any[], platform?: Platform): CommentType[] => {
  data = isArray(data) ? data : [data];
  return data.map((item) => ({
    id: item.commentId,
...
const getCoverSizeUrl = (url: string, size: number | null = null) => {
  try {
    ...
    if (imageUrl.endsWith("&")) {
      const processedUrl = imageUrl + "cl";
      return processedUrl.replace(/(thumbnail=[0-9]+y[0-9]+&cl)/, `thumbnail=${size}y${size}&`);
    }
    return imageUrl;
  } catch (error) {

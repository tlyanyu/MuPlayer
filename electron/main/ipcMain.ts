...
const initWinIpcMain = (
  ...
  ipcMain.on("set-proxy", (_, config) => {
    // 确保协议名是小写（undici 要求小写）
    const protocol = config.protocol.toLowerCase();
    const proxyRules = `${protocol}://${config.server}:${config.port}`;
    store.set("proxy", proxyRules);
    win?.webContents.session.setProxy({ proxyRules: `${config.protocol}://${config.server}:${config.port}` });
  ...
  ipcMain.handle("test-proxy", async (_, config) => {
    // 确保协议名是小写（undici 要求小写）
    const protocol = config.protocol.toLowerCase();
    const proxyUrl = `${protocol}://${config.server}:${config.port}`;
    try {
      // 动态导入 undici 的 ProxyAgent（与 Fastify 使用相同的代理方式）
      const { ProxyAgent, request } = await import("undici");
      const proxyAgent = new ProxyAgent(proxyUrl);

      // 使用 undici request + ProxyAgent 测试
      ...
      log.error("❌ Error testing proxy:", error?.message || error);
      return false;
    }
  });
  ...
  ipcMain.on("clear-all-data", async () => {
    try {
      ...
      log.info("✅ Cleared all session data successfully");
    } catch (error) {
...
const initStoreIpcMain = (store: Store<StoreType>): void => {
  ...
  // 获取后端 API 地址
  ipcMain.handle("get-api-backend", () => {
    return store.get("apiBackend", "http://localhost:3000");
  });

  // 设置后端 API 地址
  ipcMain.on("set-api-backend", (_, url: string) => {
    store.set("apiBackend", url);
    log.info(`✅ API backend updated to: ${url}`);
  });

  // 重载服务器
  ipcMain.handle("reload-server", async () => {
    try {
      // 动态导入 MainProcess 实例避免循环依赖
      const mainProcess = (await import("./index")).default;
      await mainProcess.reloadServer();
      return { success: true };
    } catch (error: any) {
